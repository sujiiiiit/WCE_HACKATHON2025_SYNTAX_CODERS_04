...
export default function DrawerDialogDemo({ onPlaceSelect }: Props) {
  const map = useMap();
    ...
    (prediction: google.maps.places.AutocompletePrediction) => {
      ...
      placesService.getDetails(
        ...
        (placeDetails: google.maps.places.PlaceResult | null) => {
          ...
          setInputValue(placeDetails?.formatted_address ?? prediction.description);
          setPredictionResults([]);
          setSessionToken(new places.AutocompleteSessionToken());
          setFetchingData(false);
        }
  ...
  const renderSearchResults = () => (
    <div className="w-full">
      {fetchingData ? (
        <div className="p-4 text-center text-color-secondary">Loading...</div>
      ) : predictionResults.length > 0 ? (
        <div className="flex flex-col">
          {predictionResults.map((prediction) => (
            <button
              key={prediction.place_id}
              className="p-3 text-left hover:bg-accent border-b border-light flex items-center"
              onClick={() => onSelect(prediction)}
            >
              <span className="mr-3 text-color-secondary">
                <svg
        ...
        <div className="p-4 text-center text-color-secondary">No results found</div>
      ) : null}
    </div>
  );
        ...
        <DialogContent className="sm:max-w-[600px] p-0 h-[60dvh] max-h-[60dvh]">
          ...
          <div className="flex border-b border-light h-12 items-center justify-between">
            ...
            <Input
              ...
              className="h-12 bg-transparent border-0 outline-0 px-4 border-x border-light rounded-none"
            />
  ...
  return (
    <Drawer>
      ...
      <DrawerContent>
        ...
        <ScrollArea className="min-h-[35dvh] max-h-[55dvh] h-dvh px-4">
          {renderSearchResults()}
        </ScrollArea>
      </DrawerContent>
    </Drawer>
